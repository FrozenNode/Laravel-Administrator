...
<script type="text/javascript">
	var site_url = "<?php echo Url::to('/') ?>",
		...
		file_url = "<?php echo URL::route('admin_display_file', array($config->name)) ?>",
		rows_per_page_url = "<?php echo URL::route('admin_rows_per_page', array($config->name)) ?>",
		...
		adminData = {
			...
			<?php if ($model) {?>
				id: <?php echo $model->exists ? $model->{$model->getKeyName()} : '0' ?>,
			<?php } ?>
